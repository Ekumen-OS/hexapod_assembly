...
#include "include/BLERemoteCharacteristic.hpp"
...
void BLERemoteCharacteristic::registerForNotify(notify_callback notifyCallback, bool notifications) {
	...
	if (notifyCallback != nullptr) {   // If we have a callback function, then this is a registration.
		...
		BLERemoteDescriptor* desc = getDescriptor(BLEUUID((uint16_t)ESP_GATT_UUID_CHAR_CLIENT_CONFIG));
		if(desc != nullptr)
		...
		BLERemoteDescriptor* desc = getDescriptor((uint16_t)ESP_GATT_UUID_CHAR_CLIENT_CONFIG);
		if(desc != nullptr)
			desc->writeValue(val, 2);
	} // End Unregister
